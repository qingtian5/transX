...
class TransXProcessor(object):
    ...
    def data_loader(config, data_set, mode="train"):
        ...
        if mode == "train":
            ...
            loader = DataLoader(tds, batch_size=config.batch_size, shuffle=True, drop_last=False, num_workers=8)
        else:
            loader = DataLoader(tds, batch_size=config.eval_batch_size, shuffle=False, drop_last=False, num_workers=8)
        return loader
