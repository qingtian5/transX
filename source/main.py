            ...
            for seed in trange(int(self.config.seed), desc="Seed"):
                ...
                for epoch in trange(int(self.config.epochs), desc="Epoch"):
                    ...
                    if self.config.do_eval and (epoch + 1) % self.config.eval_epoch == 0:
                        ...
                        model.eval()
                        mr_score = model.compute_mr(eval_data_loader, device, self.config)
                        logger.info("running eval end... MR score is {}".format(mr_score))

                        if mr_score < best_mr:
                            logger.info(f"-------------the best_mr is {mr_score}, save model--------------")
                            best_mr = mr_score
